...
def create_user(username, password):
    ...
    try:
        ...
        request = urllib.request.Request(url=url, data=binary_data)
...
def add_blog_post(title,post,tags):
    ...
    try:
        ...
        values = {"body": post, "subject": title, "tags": tags}
        data = urllib.parse.urlencode(values)
        binary_data = data.encode("utf-8")
...
def add_blog_comment(title,post):
    ...
    try:
        ...
        values = {"commentName": comment_name, "commentBody": comment_body, "permalink": permalink}
        data = urllib.parse.urlencode(values)
        binary_data = data.encode("utf-8")
